    ...
    try:
        ...
        for strVer in ["TLSv1.2","TLSv1.1","TLSv1","SSLv3"]:
            ...
            try:
                ...
                msg = 'Failure connecting to %s:%d.' % (ip,port)
                print(msg)
                number = 'v0'
                ...
                if ord(recv[0])==22:
                    ...
                    if fVuln:
                        try:
                            ...
                            fVuln = False
                    if fVuln:
                        print("[%s] %s:%d may allow early CCS" % (strVer,ip,port))
                        iVulnCount += 1
                    else:
                        print("[%s] %s:%d rejected early CCS" % (strVer,ip,port))
            else:
                print("[%s] No response from %s:%d" % (strVer,ip,port))
            try:
                s.close()
            except:
                pass
        if iVulnCount > 0:
            msg = 'There is ccs injection vul on url: ' +url+ ' ,' + ' This System Exhibits Potentially Vulnerable Behavior.\nIf this system is using OpenSSL, it should be upgraded.'
            number = 'v41'
            print(msg)
            return True,url,number,msg
        else:
            msg = 'There is no css injection vul'
            number = 'v0'
            return False,url,number,msg
